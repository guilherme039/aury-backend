...
const MacroCard: React.FC<MacroCardProps> = ({ value, goal, label, icon, onEdit }) => (
    <div className="bg-[#1A1A1D] p-4 rounded-2xl flex flex-col items-center justify-center space-y-3 border border-gray-800/50 shadow-lg shadow-black/20">
      {icon}
...
const AIInsightCard = () => (
    <div className="bg-gradient-to-r from-[#00B4D8]/10 to-[#22C55E]/10 p-4 rounded-2xl flex items-center space-x-4 border border-[#00B4D8]/20">
        <div className="p-2 bg-gradient-to-br from-[#00B4D8] to-[#22C55E] rounded-full">
             <BrainIcon className="w-6 h-6 text-white"/>
        </div>
        <div>
            <p className="font-semibold text-sm text-[#E5E7EB]">Seu consumo de proteína aumentou 18% esta semana 💪</p>
            <p className="text-xs text-[#9CA3AF]">Continue com o ótimo trabalho!</p>
        </div>
    </div>
);
  ...
  return (
    <div className="space-y-8">
      <header>
        <div className="flex items-center justify-between">
            <div className="flex items-center space-x-6">
                <button onClick={() => setActiveDay('Hoje')} className={`text-2xl font-bold transition-colors ${activeDay === 'Hoje' ? 'text-white' : 'text-gray-600 hover:text-gray-400'}`}>Hoje</button>
                <button onClick={() => setActiveDay('Ontem')} className={`text-2xl font-bold transition-colors ${activeDay === 'Ontem' ? 'text-white' : 'text-gray-600 hover:text-gray-400'}`}>Ontem</button>
            </div>
            <button onClick={onGoToSettings} className="p-2 rounded-full text-gray-600 hover:text-white transition-colors">
      ...
      <main className="space-y-8">
        <div 
          className="bg-gradient-to-br from-[#1E1E20] to-[#121214] p-6 rounded-3xl flex items-center justify-between shadow-2xl shadow-black/40"
          style={{boxShadow: '0px 10px 25px rgba(0, 0, 0, 0.4)'}}
        >
          <div className="flex flex-col">
            <p className="text-[#9CA3AF] text-sm">Restantes hoje</p>
            <div className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400" style={{textShadow: '0 0 15px rgba(255, 255, 255, 0.1)'}}>
              <AnimatedCounter from={caloriesLeft + 50} to={caloriesLeft} />
            </div>
             <div className="flex items-center justify-start gap-2 -mt-1">
                <p className="text-gray-500 text-lg">de {goals.calories} kcal</p>
                <button 
          ...
          <div className="pulse">
            <CircularProgress
              ...
              size={110}
              strokeWidth={12}
              gradient={{id: 'calories', from: '#00B4D8', to: '#22C55E' }}
              trailColor="#1A1A1D"
              centerIcon={<FireIcon className="w-8 h-8 text-[#00B4D8]" />}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <MacroCard
            ...
            goal={goals.protein}
            label="Proteína"
            icon={<ProteinGradientIcon className="w-8 h-8" />}
          ...
          <MacroCard
            ...
            goal={goals.carbs}
            label="Carbos"
            icon={<CarbsGradientIcon className="w-8 h-8" />}
          />
          <MacroCard
            value={totals.fat}
            goal={goals.fat}
            label="Gorduras"
            icon={<FatGradientIcon className="w-8 h-8" />}
          />
        </div>
